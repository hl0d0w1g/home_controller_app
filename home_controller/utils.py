...
def get_datetime(
    timedelta: Union[dict, None] = None, in_str: bool = False
...
def logging(message: str, source_module: str, source_function: str, level: str = 'INFO') -> None:
    ...
    log_msg = (
        f'[{get_datetime(in_str=True)}] - [{level}] ({source_module}/{source_function}) {message}'
    )
...
def time_str_to_time(time_str: str) -> datetime.time:
    ...
    assert (
        ...
        and len(time_str) == 5
        and TIME_RE_PATTERN.match(time_str) is not None
